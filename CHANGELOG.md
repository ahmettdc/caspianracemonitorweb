...
- **Çözüm:** `st` artık her düzenlemede **senkron** olarak cihaz-yerel `localStorage`'a aynalanıyor (`raceStateMirrorSave`, anahtar `rm_rstate_{tid}_{rid}`, şema `{stateJson, rev, dirty}`). Süreç aniden ölse bile veri kalır. **Uzlaştırma REV tabanlı** (zaman damgası DEĞİL): `openRace` yalnız ayna `dirty` (gönderilmemiş) VE uzak sürümle **aynı rev** üzerine yapılmışsa (`mirror.rev === remote.rev`) yereli geri yükler ve düzenleyicide Firebase'e geri yazar (reconcile). Başka cihaz sunucuyu ilerlettiyse (`remote.rev ≠ mirror.rev`) yerel atlanır → eski veriyle çakışma/ezme olmaz. Başarılı push'ta ayna `dirty:false` + yeni rev ile işaretlenir. Uzak okuma başarısızsa (soğuk açılış, yoğun trafik) ayna **kullanılmaz**; abonelik gerçek durumu getirir — böylece soğuk açılışta eski yerel veriyle "eskime" olmaz. Web + masaüstü, tek/çok cihaz güvenli (son-yazan-kazanır korunur). (`storage.js`, `useRaceSync.js`, `App.jsx openRace`; test: `raceStateMirror.test.js`)
  - **Not (rev tabanlı düzeltme):** İlk çözüm `updatedAt` (zaman damgası) ile uzlaştırıyordu; bu, soğuk açılışta uzak okuma boş dönünce (`remote?.updatedAt || 0` → 0) aynayı DAİMA tercih edip eski yerel veriyi gösteriyor ve geri yazıp ekip arkadaşının yeni verisini ezebiliyordu. Rev tabanlı uzlaştırma bunu giderir.
  - **Kapanışta flush (asıl/doğrudan düzeltme):** `useRaceSync` artık `visibilitychange`(hidden)/`pagehide` olaylarında bekleyen debounce'lu yazımı HEMEN gönderiyor (`flush`). Böylece normal kapatma/sekme değişiminde son düzenleme 800 ms'yi beklemeden Firebase'e ulaşıyor; yeniden açılışta sunucudan güncel değer geliyor — mirror kurtarmasına gerek kalmadan. Mirror yalnız ani/zorla kapanma (süreç öldürme) için güvenlik ağı olarak kalıyor.
