...
export const CHANGELOG = [
  {
    ...
    tr: [
      ...
      "🎨 Telemetri ekranı tasarım fişine hizalandı: Stint kartlarında artık marka logosu ve araç görseli var; Seans kutusuna 'Bu seansın setup'ı' butonu eklendi (tıklayınca o seansın setup içeriği açılıp görünüme kayıyor); Setup kategorilerinde kalan emojiler (💡 Elektronik, 🛢 Motor) çizgi ikonlarla değişti ve yanlış eşlenmiş Süspansiyon/Diferansiyel ikonları düzeltildi. Ayrıca telemetri grafikleri (ızgara, eksen, tooltip) uygulamanın kendi renk paletine geçti — daha önce başka bir uygulamadan gelmiş gibi duran mavi-gri bir palet kullanıyorlardı.",
    ...
    en: [
      ...
      "🎨 The Telemetry screen now matches the design spec: stint cards show the brand logo and car image; the Session panel gained a \"This session's setup\" button (it opens that session's setup contents and scrolls them into view); the leftover emoji in setup categories (💡 Electronics, 🛢 Engine) became line icons, and the mis-mapped Suspension/Differential icons were corrected. The telemetry charts (grid, axes, tooltip) also moved onto the app's own colour palette — they had been using a blue-grey palette that looked like it came from a different app.",
