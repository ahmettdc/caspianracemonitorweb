...
export const CHANGELOG = [
  {
    ...
    date: "2026-09-01",
    tr: [
      "🛞 TUR BAŞI AŞINMA geldi: lastik sekmesinde artık her köşenin KENDİ aşınma hızı var. Şimdiye kadar \"ölçülen aşınma\" tek bir sayıydı ve iki şeyi birden gizliyordu. Birincisi dört köşeyi \"en kötüsü\"ne indiriyordu — sağ virajı bol bir pistte ön-sol lastiğin tek başına bitmesi ekranda hiç görünmüyordu. İkincisi hızı anlık dişten çıkardığı için stint ORTALAMASIydı; lastik düşüşünün başlayıp başlamadığı okunamıyordu. Artık köprü her tamamlanan turun dört köşe dişini kaydediyor, ekran da bundan GERÇEK tur farkını hesaplıyor: köşe başına %/tur, kalan diş ve \"bu lastikle kaç tur kaldı\". Son 5 turdaki hız dönem ortalamasından belirgin saparsa \"↑ hızlanıyor\" / \"↓ yavaşlıyor\" yazıyor — degradasyonun döndüğü an tam olarak budur. Başlıkta pit penceresini belirleyen köşe ayrıca gösteriliyor, çünkü durağı o köşe zorlar. \"Stint başına aşınma\" önerisi de artık bu gerçek ölçümden besleniyor (eski anlık ölçüm, kayıt henüz birikmemişken yedek olarak duruyor).",
      "🔍 Aynı özellikte iki dürüstlük kaydı: (1) aşınma HIZI gerçek okumadır (turlar arası diş farkı), ama KALAN TUR hızın sabit kalacağı varsayımıyla modellenmiş tahmindir — ekranda da böyle etiketlendi, çünkü degradasyon çoğu zaman doğrusal değildir. (2) Lastik değişimi serinin KENDİSİNDEN okunuyor: bir köşenin dişi yukarı sıçradıysa o köşe değişmiştir. Pit kaydından tahmin edilmedi, çünkü oyun orada yalnız \"kaç lastik\" veriyor, HANGİ köşe olduğunu vermiyor — bu yüzden 2 lastik değiştiğinde yalnız gerçekten değişen köşeler sıfırlanıyor, diğer ikisinin geçmişi korunuyor. Online yarışta oyun rakiplerin aşınmasını yayınlamadığı için kayıt kendi aracınızda (ve offline/AI yarışlarda herkeste) tutulur; veri yoksa satır çizilmez, tahmin üretilmez.",
    ],
    en: [
      "🛞 WEAR PER LAP has arrived: every corner now has its OWN wear rate in the Tyres tab. Until now \"measured wear\" was a single number that hid two things at once. First, it collapsed all four corners into the \"worst\" one — a front-left going off on its own at a right-hander-heavy track was simply invisible. Second, it derived the rate from the instantaneous tread, so it was a stint AVERAGE; you could not see whether drop-off had started. The bridge now records all four corner treads for every completed lap, and the screen computes the REAL lap-to-lap difference from them: %/lap per corner, tread left, and \"how many laps are left on this tyre\". If the rate over the last 5 laps deviates clearly from the period average it says \"↑ accelerating\" / \"↓ easing off\" — that is exactly the moment degradation turns. The corner that sets the pit window is called out in the header, because that is the corner that forces the stop. The \"wear per stint\" suggestion is now fed from this real measurement too (the old instantaneous reading remains as a fallback until records build up).",
      "🔍 Two honesty notes on the same feature. (1) The wear RATE is a real reading (tread difference between laps), but LAPS LEFT is a modelled projection assuming the rate stays constant — labelled as such on screen, because degradation is usually not linear. (2) A tyre change is read from the series ITSELF: if a corner\u2019s tread jumps back up, that corner was replaced. It is not inferred from the pit record, because the game only gives \"how many tyres\" there, never WHICH corner — so when 2 tyres are changed only the corners that actually changed are reset, and the other two keep their history. In online races the game does not broadcast rivals\u2019 wear, so the record is kept for your own car (and for everyone in offline/AI races); with no data no row is drawn and nothing is guessed.",
    ],
  },
  {
    v: "v2.3.0",
